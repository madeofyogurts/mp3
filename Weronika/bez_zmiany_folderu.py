...
#print("Your music folder location: ", get_path())

playlist = []
for (path, subdirs, files) in os.walk(get_path()):
    for f in files:
        if '.mp3' in f:
...
m = 0
print("\n Your playlist now contains following files:")
...
class MyPanel(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        user_whereabouts_label = label_creation(self, path, 17, 99, 79, '#FF2D55')

        song = playlist[n][playlist[n].rfind("//")+2:len(playlist[n])+1]
        song_title_label = label_creation(self, 'Song Title', 22, 99, 123, '#000000', 'bold')
        artist_label = label_creation(self, song, 17, 99, 157, '#707070')
